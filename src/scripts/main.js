...
pianoMods.forEach((mod, index) => {
  ...
  pianoModContainer.className =
    "piano__control-panel__mode-panel-holder__mode-holder";
...
pianoMelodyContainer.className =
  "piano__control-panel__mode-panel-holder__melody-container";
...
pianoMelodyLabel.className =
  "piano__control-panel__mode-panel-holder__melody-container__piano-melody-label";
pianoMelodyContainer.append(pianoMelodyLabel);

const pianoMelodyInput = document.createElement("div");
pianoMelodyInput.className =
  "piano__control-panel__mode-panel-holder__melody-container__piano-melody-editor";
...
function renderMelody() {
  ...
  melody.notes.forEach((note, index) => {
    pianoMelodyInput.append(generateNotesElement(note, index));
  });
}
...
function generateNotesElement(note, index) {
  ...
  noteElement.className =
    "piano__control-panel__mode-panel-holder__melody-container__piano-melody-editor__note";

  const noteLabel = document.createElement("label");
  noteLabel.textContent = note.getKeyLabel();
  noteLabel.className =
    "piano__control-panel__mode-panel-holder__melody-container__piano-melody-editor__note__label";
  noteElement.append(noteLabel);

  const crossIconContainer = document.createElement("div");
  crossIconContainer.className =
    "piano__control-panel__mode-panel-holder__melody-container__piano-melody-editor__note__cross-icon";
...
pianoMelodyEditButton.className =
  "piano__control-panel__mode-panel-holder__melody-container__piano-melody-edit-button";
...
pianoMelodyPlayButton.className =
  "piano__control-panel__mode-panel-holder__melody-container__piano-melody-play-button";
...
function addReassignIcon(keyElement) {
  const reassignKeyIcon =
    reassignIconSvgDocument.documentElement.cloneNode(true);
  reassignKeyIcon.classList.add(
    "piano__keys-holder__light-key__reassign-key-icon",
  );
...
for (let i = 0; i < lightKeysCount; i += 1) {
  ...
  lightKeyAssignedKeyLabel.className =
    "piano__keys-holder__light-key__assigned-key-label";
  lightKeyAssignedKeyLabel.textContent = assignedKey
    ? assignedKey.getKeyLabel()
    : "";
...
async function openAssignKeyModal(keyElement, keyLabelElement) {
  ...
  async function showConfirmMessage(goal, additionalMessage = undefined) {
    ...
    confirmMessage.textContent = additionalMessage
...
darkKeys.forEach((darkKey, i) => {
  ...
  darkKeyAssignedKeyLabel.className =
    "piano__keys-holder__dark-key__assigned-key-label";
  darkKeyAssignedKeyLabel.textContent = assignedKey
    ? assignedKey.getKeyLabel()
    : "";
